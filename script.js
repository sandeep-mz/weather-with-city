...
// Add an event listener for the 'DOMContentLoaded' event to load initial data when the page is loaded.
document.addEventListener("DOMContentLoaded", () => {
  loadInitialCities();
});

// Function to load initial city weather when the page is loaded
function loadInitialCities() {
  ...
  initialCities.forEach(async (city) => {
    await driverFun(city);
  });
}

async function driverFun(city) {
  try {
    // Remove existing data for all cities
    citiesWeather.splice(0, citiesWeather.length);
